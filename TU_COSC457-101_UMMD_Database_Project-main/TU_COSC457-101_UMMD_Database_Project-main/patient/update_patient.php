...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        // Update patient data
        $sql = "UPDATE Patients SET first_name=?, last_name=?, birth_date=?, gender=?, phone=?, address=?, insurance_id=? WHERE patient_id=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssssii", $first_name, $last_name, $birth_date, $gender, $phone, $address, $insurance_id, $patient_id);
        ...
        $conn->rollback();
        // Redirect back to edit patient form with an error message (optional)
        header("Location: edit_patient.php?id=$patient_id&error=" . urlencode($e->getMessage()));
        exit();
    }
